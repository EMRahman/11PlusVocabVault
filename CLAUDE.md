...
| `quest-levels.js` | ~150 | Pure Constellation Quest level logic (`selectClusterBatch` — the extracted hub-first/synonym-BFS/proximity word picker; `clusterRemaining`, `isCorpusExhausted`, `clustersWithWordsRemaining`, `countCapturedPerCluster`, `deriveBeaconStates`). State passed in; shuffle injectable for golden tests. | imported by word-quest-3d.js |
| `word-universe.js` | ~490 | Three.js 3D word cloud visualisation. | `<script type="module">` (own tag) |
| `word-quest-3d.js` | ~880 | Constellation Quest 3D game. **Progressive levels:** clearing all 12 clusters opens "Continue to Level N+1", which replays the same map serving the next batch of uncaptured words (`capturedWords` carried across levels) until all 437 words are captured (~15 levels → grand finale). Journey is saved/resumable in `vocabVault_constellationQuest` (`progress` snapshot: level/score/capturedWords/clearedThisLevel; v2 schema, pre-v2 saves load as Level 1). | `<script type="module">` (own tag) |
